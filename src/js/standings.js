jQuery(document).ready(function() {
  ...
  for (i = Drupal.settings.today.season_year; i > 2015; i--) {
    if (i == Drupal.settings.today.season_year) {
      ...
      jQuery(".season-select").append(
        '<option value="' + i + '">' + i + "-" + (i + 1) + " SEASON</option>"
      );
    }
  }
            ...
            for (o = 0; o < index.length; o++) {
              if (o == 0) {
                rowHTML +=
                  ...
                  '><p class="seed">' +
                  standingsData.sta.co[i].di[x].t[t].see +
                  '</p><div class="logo-wrap"><img class="logo" src=http://i.cdn.turner.com/nba/nba/assets/logos/teams/primary/web/' +
                  standingsData.sta.co[i].di[x].t[t].ta +
                  '.svg></div><p class="team-name">' +
                  standingsData.sta.co[i].di[x].t[t].tc.toUpperCase() +
                  " " +
                  standingsData.sta.co[i].di[x].t[t].tn.toUpperCase() +
                  '</p><p class="team-abbreviation">' +
                  standingsData.sta.co[i].di[x].t[t].ta +
                  "</p></td>";
              } else {
                rowHTML +=
                  "<td>" +
                  standingsData.sta.co[i].di[x].t[t][index[o]] +
                  "</td>";
              }
            }
            jQuery(conferences[i] + " tr:nth-child(" + (place + 2) + ")").html(
              rowHTML
            );
            jQuery("tr").removeClass("active");
            if (standingsData.sta.co[i].di[x].t[t].ta == "BOS") {
              jQuery(
                conferences[i] + " tr:nth-child(" + (place + 2) + ")"
              ).addClass("active");
            }
          }
        }
